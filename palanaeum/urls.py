...
urlpatterns = [
  ...
  re_path('static_page/(?P<page_name>[^/]+/)', views.static_page, name="static_page"),
  ...
  path('events/<int:event_id>/source/choose_type/', palanaeum.staff_views.choose_source_type,
       name='event_choose_source_type'),
  ...
  path('events/<int:event_id>/sort_entries/snippets/', staff_views.reorder_entries_by_snippets,
       name="sort_entries_by_snippets"),
  path('events/<int:event_id>/sort_entries/creation_date/',
       staff_views.reorder_entries_by_creation_date, name="sort_entries_by_creation_date"),
  path('events/<int:event_id>/sort_entries/date/', staff_views.reorder_entries_by_assigned_date,
       name="sort_entries_by_assigned_date"),
  ...
  path('snippet/<int:snippet_id>/edit_entry/', staff_views.edit_snippet_entry,
       name='edit_snippet_entry'),
  path('snippet/<int:snippet_id>/create_entry/', staff_views.create_entry_for_snippet,
       name='create_entry_for_snippet'),
  ...
  path('source/image/<int:source_id>/assign_entry/', staff_views.edit_image_source_entry,
       name="edit_image_entry"),
  path('source/image/<int:source_id>/create_entry/', staff_views.create_entry_for_image_source,
       name='create_entry_for_image_source'),
  path('source/image/<int:source_id>/delete/', staff_views.remove_image_source,
       name='remove_image_source'),
  path('source/image/<int:source_id>/rename/', staff_views.rename_image_source,
       name='rename_image_source'),

  re_path(r'^source/(?P<source_type>audio|image)/(?P<pk>\d+)/approve/', staff_views.approve_source,
          name='approve_source'),
  re_path(r'^source/(?P<source_type>audio|image)/(?P<pk>\d+)/reject/', staff_views.reject_source,
          name='reject_source'),
  ...
  path('admin/realted_sites/<int:site_id>/', admin_views.related_site_edit,
       name="admin_related_site_edit"),
  path('admin/realted_sites/edit/', admin_views.related_site_edit, name="admin_related_site_new"),
  path('admin/realted_sites/<int:site_id>/delete/', admin_views.related_site_delete,
       name="admin_related_site_delete"),
  ...
  path('staff/new_static_page/', staff_views.edit_static_page, name='new_static_page'),
  path('staff/static_page/<int:page_id>/', staff_views.edit_static_page, name='edit_static_page'),

  path('auth/login/', auth_views.LoginView.as_view(template_name='palanaeum/auth/login.html'),
       name='auth_login'),
  path('auth/logout/', auth_views.LogoutView.as_view(next_page='/'), name='auth_logout'),
  path('auth/password_change/', auth_views.PasswordChangeView.as_view(**password_change_kwargs),
       name='auth_password_change'),
  path('auth/password_change/done/', views.password_change_complete, name='auth_password_change_done'),
  path('auth/password_reset/', auth_views.PasswordResetView.as_view(**password_reset_kwargs),
       name='auth_password_reset'),
  path('auth/password_reset/done/',
       auth_views.PasswordResetCompleteView.as_view(**password_reset_done_kwargs),
       name='auth_password_reset_done'),
  re_path(r'^auth/reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
          auth_views.PasswordResetConfirmView.as_view(**password_reset_confirm_kwargs),
